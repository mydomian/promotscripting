                              ...
                              <div class="card marketplace--card rounded-3">
                                      ...
                                      <div class="card-body">
                                          <div class="d-flex align-items-center mb-3">
                                              ...
                                              <a href="javascript:;" class="text-white text-decoration-none fw-semibold" style="padding-left: 10px;"> <small> {{ $purchase->product->user ? $purchase->product->user->name : ''  }}</small></a>
                                          </div>
                                          <a
                                          href="{{ route('marketplaceDetails',['slug'=>Str::slug($purchase->product->title,'-'),'product'=>$purchase->product->id]) }}"
                                          ...
                                          @endphp
                                          <img src="@if($purchase->product->image) {{ asset('/storage/products/thumbnil/'.$purchase->product->image) }} @else https://picsum.photos/200/300 @endif" alt="{{ $slug }}" class="img-fluid w-100 rounded-3 object-fit-cover"/>
                                          <span class="bg-dark mx-2 mt-2 text-white text-center opacity-50" style="height: 25px;">{{ $purchase->product->subSubCategory->subCategory->category ? $purchase->product->subSubCategory->subCategory->category->category_name : $purchase->product->subSubCategory->category_name }}</span>
                                          </a>
                                          <div class="d-flex justify-content-between gap-3">
                                              <a href="{{ route('marketplaceDetails',['slug'=>Str::slug($purchase->product->title,'-'),'product'=>$purchase->product->id]) }}" class="link-light text-decoration-none">{{ Str::limit($purchase->product->title,22) ?? "" }}</a>
                                              <small class="text-body-tertiary">$ {{ $purchase->product->price }}</small>
